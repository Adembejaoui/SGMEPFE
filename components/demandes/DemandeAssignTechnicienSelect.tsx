...
interface DemandeAssignTechnicienSelectProps {
  demandeId: number
...
export function DemandeAssignTechnicienSelect({ demandeId, currentTechnicienId, equipementType, onAssigned }: DemandeAssignTechnicienSelectProps) {
  ...
  const fetchTechniciens = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const filtered = data.filter((t: TechnicianOption) => t.specialization === equipementType)
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
            Assigner un technicien ({equipementType})
          </label>
          <Select
            value={selectedId}
